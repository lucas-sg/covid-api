...
class CovidService:
    ...
    def summary(cls, group_by_vector, start_date, end_date, data):
        ...
        if not end_date:
            query = "SELECT max(ultima_actualizacion) FROM covid19_casos;"
            end_date = CovidService.get_data(query).data_frame['max'].values[0]
    ...
    @classmethod
    def replace_value_tuple(cls, idx, tup, value):
        aux = list(tup)
        aux[idx] = value
        return tuple(aux)
    ...
    def dump_csv_lines_into_db(cls, connection, csv_url, chunk_size):
        ...
        cur.execute("DELETE FROM public.covid19_casos WHERE id_evento_caso > 0;")
        connection.commit()
        ...
        for csv_chunk in cls.read_csv_chunks(csv_url, chunk_size):
            ...
            for index, tup in enumerate(clean_chunk):
                if tup[2] == '':
                    clean_chunk[index] = cls.replace_value_tuple(2, clean_chunk[index], None)
                if tup[8] == '':
                    clean_chunk[index] = cls.replace_value_tuple(8, clean_chunk[index], None)
                if tup[9] == '':
                    clean_chunk[index] = cls.replace_value_tuple(9, clean_chunk[index], None)
                if tup[10] == '':
                    clean_chunk[index] = cls.replace_value_tuple(10, clean_chunk[index], None)
                if tup[11] == '':
                    clean_chunk[index] = cls.replace_value_tuple(11, clean_chunk[index], None)
                if tup[13] == '':
                    clean_chunk[index] = cls.replace_value_tuple(13, clean_chunk[index], None)
                if tup[15] == '':
                    clean_chunk[index] = cls.replace_value_tuple(15, clean_chunk[index], None)
                if tup[17] == '':
                    clean_chunk[index] = cls.replace_value_tuple(17, clean_chunk[index], None)
                if tup[21] == '':
                    clean_chunk[index] = cls.replace_value_tuple(21, clean_chunk[index], None)
                if tup[22] == '':
                    clean_chunk[index] = cls.replace_value_tuple(22, clean_chunk[index], None)
                if tup[23] == '':
                    clean_chunk[index] = cls.replace_value_tuple(23, clean_chunk[index], None)
                if tup[24] == '':
                    clean_chunk[index] = cls.replace_value_tuple(24, clean_chunk[index], None)
